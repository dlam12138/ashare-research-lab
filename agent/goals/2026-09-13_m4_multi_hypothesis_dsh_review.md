...
## 2026-09-13 resume addendum: one small closeout

User authorized continuing after the recorded `BLOCKED` DSH attempt, while retaining the preference to stop after one small stage. Start from the verified clean `codex/m4-multi-hypothesis-acceptance@654de8f238fa17738c487c50cf50d630ae5b7569`; main and protected identities above are unchanged. Preserve the historical blocked record and acceptance. This addendum authorizes only `agent/record/2026-09-13_02_m4-multi-hypothesis-dsh-review.md` and a new `acceptance/2026-09-13_m4_multi_hypothesis_dsh_resume.md` as DSH delivery paths; do not modify tests or product code. Codex commits this addendum and initial record note before DSH resumes.

Do **not** create, enumerate or delete another temporary directory, and do not repeat sandbox diagnostics. DSH should run the two tests that do not use `TemporaryDirectory`, then reproduce both fixed fingerprints in a fresh child Python process from an existing, different CWD. Use the same `_request`, `_fingerprint`, `EXPECTED_FINGERPRINTS` and `run_synthetic_pipeline` from the new test, with `PYTHONPATH` pointing to this worktree's `src` and `tests`. The child process must assert equality to the fixed values and exit 0; this is a direct substitute for the DSH-blocked cleanup wrapper, not a claim that the original full three-test command passed inside DSH. Run Ruff on the unchanged new test and check Git hygiene. Codex's previous host rerun of the original three-test command (`3 passed`) remains separately identified host evidence.

If these bounded checks pass, document a limited `PASS` for the *resumed local DSH audit*, with the earlier DSH `BLOCKED` result and ignored empty `tmp/dsh-temp/tmpqb9tc2rv` still disclosed. If they fail, report the actual failure and stop. Actual Ubuntu/Windows CI remains pending; no push, PR, merge or next stage. Codex independently checks final files, statuses, remote, stash and database before accepting.
