...
| AC-10 | 篡改：合同字段被改（摘要未改） | 篡改 | S1 | 上游篡改必须 fail-closed |
| AC-11 | 篡改：合同字段被改且摘要被重写 | 篡改 | S2 | 重编译一致性是第二道检测 |
| AC-12 | 篡改：计划字段/摘要被改 | 篡改 | S3 | 计划是绑定对象，不可改写 |
| AC-13 | 篡改：`bound_inputs` 三处摘要与观测级篡改 | 篡改 | S3–S4 | 输入不可伪造 |
| AC-14 | 篡改：准备数据与矩阵逐层重哈希伪造 | 篡改 | S5 | 重算 + 逐字节比对 |
| AC-15 | 篡改：执行产物与信封逐层篡改 | 篡改 | S6 / V3–V6 | 全链身份覆盖到底 |
| AC-16 | 首错：多故障并存时唯一的首错归属 | 错误顺序 | S0–S3 | 顺序是规范，不是实现细节 |
| AC-17 | 首错：阶段内错误码透传且不被改写 | 错误顺序 | S1–S6 | 编排器不吞错、不改码 |
...
| AC-22 | 结构禁令：路径、seed、排序与选择字段不可表达 | 结构性禁令 | S0 / V5 | 禁止键是闭集 |
...
| AC-28 | 边界：`replications` 资源上界门 | 边界 | S3 / G11（G10 后） | 资源界不等于统计充分性 |
...
合同/计划/准备数据/矩阵/执行产物提交给 `run_synthetic_pipeline`。全部篡改案例因此必须
标明其**可达入口**：

| 类别 | 可达入口 | 案例 |
| --- | --- | --- |
| 信封层篡改 | `validate_pipeline_result(forged_result, request)`（公开，V2/V6） | AC-15e、AC-15f |
| 请求层篡改 | `run_synthetic_pipeline(request=...)`（公开，S0–S6） | AC-10、AC-12b、AC-13、AC-16、AC-17c、AC-17d |
| 阶段级探针 | **非**组合入口；直接调用既有阶段验证器，必须在本节显式标注 | AC-11、AC-12a、AC-12c、AC-14、AC-15a–d、AC-17a、AC-17b、AC-17e、AC-17f |

**阶段级探针的标注要求**：这些案例**不是**组合入口的验收，而是"既有阶段仍然 fail-closed"
的回归证据。实现阶段必须把它们写成显式命名的独立测试，并在测试名或注释中标注
`STAGE_LEVEL_PROBE`。**不得**把它们伪装成组合入口的行为。
...
`contract_digest`，把该对象直接作为下游输入（模拟"绕过 S1"的调用路径）。
...
**授权解释**：上游篡改必须 fail-closed；摘要不是装饰。
...
| 子案例 | 变化 | 预期 |
| --- | --- | --- |
| AC-12a | 改计划语义字段，保留 `plan_digest` | `validate_analysis_plan` → `PLAN_DIGEST_MISMATCH` |
| AC-12b | 改计划字段并用 `compute_plan_digest` 重写摘要 | S3 的 `bound_inputs.plan_digest` 不符 → `PIPELINE_INPUT_BINDING_MISMATCH`；若调用者同步改了 `bound_inputs`，则由适配器 `CONTRACT_PLAN_MISMATCH` 拒绝 |
| AC-12c | 只改 `plan.source_contract_digest` | 形态仍合法但重编译比对失败 → 既有 `ValueError` |

**预期结果**：任一子案例都在 S2/S3 终止，不进入 S5/S6。

**验证阶段**：S2–S3。

**授权解释**：计划是绑定对象，不可改写；输入摘要不能"跟随"被篡改的计划。
...
**预期结果**：四个子案例全部在 S5 终止，**不**进入 S6，**不**产生任何统计量。

**验证阶段**：S5；V2。

**授权解释**：重算 + 逐字节比对使"自洽的伪造"也不可表达。
...
AC-15e/f 可经公开的 `validate_pipeline_result(forged_result, request)` 到达（第 1.2 节）。
组合入口本身**不**接收执行产物。

**验证阶段**：S6（e/f 为 V2/V6）；阶段级（a–d）。
...
| 子案例 | 触发方式 | 预期类型与码 |
| --- | --- | --- |
| AC-17a | 合同 `holdout_policy` 与开发期重叠 | `ContractCompilationError("DEVELOPMENT_HOLDOUT_OVERLAP")` |
| AC-17b | 合同方法非 `DAILY_CONDITIONAL_CONTROLLED_OLS_V1` | 既有 `ValueError("UNSUPPORTED_ANALYSIS_METHOD")` |
| AC-17c | 绑定 role 序列与计划不符 | `AdapterError("ROLE_BINDING_MISMATCH")` |
| AC-17d | `input_digest` 位级错误 | `AdapterError("INPUT_DIGEST_MISMATCH")` |
| AC-17e | 矩阵角色项与计划不符 | `MatrixError("PLAN_TERM_ROLE_MISMATCH")` |
| AC-17f | 执行输入含禁止键或绝对路径值 | `ExecutionError("FORBIDDEN_ARTIFACT_CONTENT")` |
...
**授权解释**：编排器不吞错、不改码；错误面增量最小。AC-17a/b/e/f 为**阶段级探针**
（第 1.2 节），不得伪装成组合入口行为。
...
| AC-22d | 合同声明的稳健性参数值含 `/`（如 `"A/B"`）或参数键为 `"path"` | `FORBIDDEN_ARTIFACT_CONTENT`——这是**既有**执行层行为，不是编排器引入的缺陷 |
...
7. **claim boundary**：处于执行态的记录**不可**绑定（AC-25d）；绑定**不**使信封出现任何
   "已执行/已建立/已提升"字段或取值。
...
6. 变更路径集合恰为设计第 12.3 节/Goal 白名单的 6 条路径。
...
| AC-28d | `enabled=false`（`method_id="DISABLED"`） | 成功；不建立 generator；上界门不适用 |
...
| AC-29d | 8，且调用失败（任一 AC-10/AC-13 路径） | 调用前后 `decimal.getcontext()` 的 `prec`/`traps`/`flags` 逐字段不变（编排器**不得**写全局上下文） |
...
2. 因此"用一份伪造的合同/计划/矩阵去驱动编排器"**在类型层不可表达**——这是比任何运行时
   校验都强的结构性保证；
3. 需要验证伪造上游对象时，唯一的公开入口是
   `validate_pipeline_result(forged_result, request)`（信封层，AC-15e/f）或
   显式标注的阶段级探针（AC-11/12a/12c/14/15a–d/17a/17b/17e/17f）；
...
4. 再断言**篡改检测**：AC-10 – AC-15，逐层从下到上。AC-11 与 AC-15f 是两条
   "自洽伪造"的关键反例，必须在实现中被真实复现。按第 1.2 节把 AC-11/12a/12c/14/15a–d/
   17a/17b/17e/17f 写成显式标注 `STAGE_LEVEL_PROBE` 的独立测试，**不得**伪装成组合入口行为。
...
6. 再断言**正常与确定性**：AC-01、AC-02、AC-03、AC-04、**AC-29**。AC-03d 的跨 numpy 版本差异
   ...
   （因为编排器必须自己固定 `prec`）。
7. 最后断言**有界资源**：设计第 11.3 节的调用次数上界必须被实测并收紧；
   超出即设计缺陷，须回到设计评审而**不是**放宽上界。
...
- 编排入口
  ...
  **尚不存在**；本文**不声称**任何编排器测试已通过。AC-01 – AC-30 在实现前**不可执行**；
  其中 AC-11/12a/12c/14/15a–d/17a/17b/17e/17f 明确标注为阶段级探针（第 1.2 节）。
