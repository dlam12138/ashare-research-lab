"""Regression tests for the committed 2022 annual evidence bundle.
...
def test_registered_2022_bundle_validates():
    validate_bundle(_load(2022))


def test_registered_2022_bundle_uses_annual_cas_consolidated_zh_cn():
    bundle = _load(2022)
    assert (
        bundle["report_type"],
...
def test_registered_2022_bundle_has_exact_two_official_sources():
    documents = _load(2022)["documents"]
...
def test_registered_2022_bundle_has_exact_six_facts():
    assert len(_load(2022)["facts"]) == 6


def test_registered_2022_bundle_has_exact_three_concepts_per_source():
    facts = _load(2022)["facts"]
...
def test_registered_2022_source_ids_are_distinct():
    documents = _load(2022)["documents"]
    assert documents["company"]["source_id"] != documents["exchange"]["source_id"]


def test_registered_2022_values_normalize_without_float():
    for registered in _load(2022)["facts"]:
...
def test_registered_2022_builds_six_canonical_source_facts():
    facts = [fact for group in _facts(_load(2022)).values() for fact in group]
    assert len(facts) == 6
    assert all(fact["fact_id"] == build_fact_id(fact) for fact in facts)


def test_registered_2022_three_pairs_match_in_memory():
    facts = _facts(_load(2022))
    by_source = {
...
def test_registered_2022_fact_ids_differ_from_later_years():
    ids_by_year = {
        year: {
    ...
    assert all(
        ids_by_year[2022].keys() == ids_by_year[year].keys()
        for year in (2023, 2024, 2025)
    )
    assert all(
        ids_by_year[2022][key] != ids_by_year[year][key]
        for year in (2023, 2024, 2025)
        for key in ids_by_year[2022]
    )


def test_registered_2022_available_at_uses_later_announcement():
    bundle = _load(2022)
    facts = _facts(bundle)
...
def test_registered_2022_context_is_annual_duration():
    context = build_context(_load(2022), created_at="test")
    assert context["context_id"] == "601857.SH|2022|annual|consolidated"
    assert context["instant_or_duration"] == "duration"


def test_registered_2022_bundle_contains_no_local_absolute_paths():
    text = json.dumps(_load(2022), ensure_ascii=False)
