"""Regression tests for the committed 2021 annual evidence bundle.
...
def _facts(bundle: dict) -> dict[str, list[dict]]:
    return build_source_facts(bundle, created_at="2026-07-30T00:00:00")


def test_registered_2021_bundle_validates():
    validate_bundle(_load(2021))


def test_registered_2021_bundle_uses_annual_cas_consolidated_zh_cn():
    bundle = _load(2021)
    assert (
        bundle["report_type"],
...
def test_registered_2021_bundle_uses_scoped_document_relationship():
    bundle = _load(2021)
    assert bundle["document_relationship"] == (
        "audited_financial_statements_subset_of_full_annual_report"
    )
    assert (
        bundle["documents"]["company"]["document_scope"]
        == AUDITED_FINANCIAL_STATEMENTS
    )
    assert bundle["documents"]["exchange"]["document_scope"] == FULL_ANNUAL_REPORT


def test_registered_2021_bundle_has_exact_two_official_sources():
    documents = _load(2021)["documents"]
...
def test_registered_2021_bundle_has_exact_six_facts():
    assert len(_load(2021)["facts"]) == 6


def test_registered_2021_bundle_has_exact_three_concepts_per_source():
    facts = _load(2021)["facts"]
...
def test_registered_2021_values_normalize_without_float():
    for registered in _load(2021)["facts"]:
...
def test_registered_2021_builds_six_canonical_source_facts():
    facts = [fact for group in _facts(_load(2021)).values() for fact in group]
    assert len(facts) == 6
    assert all(fact["fact_id"] == build_fact_id(fact) for fact in facts)


def test_registered_2021_three_pairs_match_in_memory():
    facts = _facts(_load(2021))
    by_source = {
...
def test_registered_2021_fact_ids_differ_from_later_years():
    ids_by_year = {
        year: {
    ...
    assert all(
        ids_by_year[2021].keys() == ids_by_year[year].keys()
        for year in range(2022, 2026)
    )
    assert all(
        ids_by_year[2021][key] != ids_by_year[year][key]
        for year in range(2022, 2026)
        for key in ids_by_year[2021]
    )


def test_registered_2021_available_at_uses_later_announcement():
    bundle = _load(2021)
    facts = _facts(bundle)
...
def test_registered_2021_context_is_annual_duration():
    context = build_context(_load(2021), created_at="test")
    assert context["context_id"] == "601857.SH|2021|annual|consolidated"
    assert context["instant_or_duration"] == "duration"


def test_registered_2021_manifest_keeps_sources_non_independent():
    manifest = _public_source_manifest(
        _load(2021),
        {"company": {"verified": True}, "exchange": {"verified": True}},
    )
    assert manifest["independent_content_sources"] is False
    assert (
        manifest["documents"]["company"]["document_scope"]
        == AUDITED_FINANCIAL_STATEMENTS
    )
    assert manifest["documents"]["exchange"]["document_scope"] == FULL_ANNUAL_REPORT


def test_registered_2021_bundle_contains_no_local_absolute_paths():
    text = json.dumps(_load(2021), ensure_ascii=False)
