...
def test_contracts_are_valid_and_constants_are_frozen() -> None:
    pipeline = load_json_contract("m3_stage3ca_analysis_pipeline_contract_v2.json")
    model = load_json_contract("m3_stage3ca_model_specification_v2.json")
    robustness = load_json_contract("m3_stage3ca_robustness_registry_v2.json")
    output = load_json_contract("m3_stage3ca_output_schema_v2.json")
...
def test_production_bootstrap_contract_is_5000_replicates() -> None:
    contract = load_json_contract("m3_stage3ca_analysis_pipeline_contract_v2.json")
...
def test_digest_payload_binds_upstream_identity_and_dependencies() -> None:
    ...
    with pytest.raises(UpstreamBindingError, match="MODEL_DIGEST_UPSTREAM_BINDING_FAILURE"):
        build_model_digest(
            ...
            contract_paths=[root / "reports/m3_stage3ca_model_specification_v2.json"],
    ...
    digest, payload = build_model_digest(
        ...
        upstream_inventory_path=(
            root / "reports/m3_stage3car_effective_upstream_sha_inventory_v1.json"
        ),
    )
    assert len(digest) == 64
    assert "M3_STAGE3A_MECHANISM_HYPOTHESIS_CONTRACT_V1" in payload[
...
def test_cli_writes_only_synthetic_machine_outputs(tmp_path: Path) -> None:
    ...
    assert result["status"] == "M3_STAGE3CAR_CANONICAL_RECOVERY_ACCEPTED"
...
def test_primary_output_contains_every_locked_field() -> None:
    ...
    required = load_json_contract("m3_stage3ca_output_schema_v2.json")[
